...
function AdminProfileComponent() {
  ...
  useEffect(() => {
    ...
    // If not logged in, redirect to Logins page
    if (!isLoggedIn) {
      navigate('/Logins'); // Replace '/Logins' with the actual Logins route
    }
  }, [navigate]);
  ...
  function handleEdit(eventId) {
    ...
    // Find the booking data for the selected eventId
    const selectedBooking = bookingData.find(booking => booking.eventId === eventId);
    ...
    setEditFormData(
      {
        ...
        "venue": {
          "venueId": selectedBooking.venue.venue_id,
        ...
        "catering": {
          "cateringId": selectedBooking.catering.catering_id,
          "menu": selectedCatering
        },
        "decoration": {
          "decorationId": selectedBooking.decoration.decoration_id,
          "decorType": selectedDecoration
        },
        "media": {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(bookingData);
    console.log(parseInt(bookingData[0].eventId));
  ...
  function custDataHandler(emailId) {
    axios.get(`/Accounts/${emailId}`)
      .then(response => {
              ...
              <tr key={bData.eventId}>
                ...
                <td>
                  <button
                    ...
                    onClick={() => handleEdit(bData.eventId)}
                  >
                    Edit
                  </button>
                </td>
                <td>
                  <td>
                    <button
                      ...
                      onClick={() => {
                        setEventToDelete(bData.eventId);
                        // handleDelete(bData.eventId);
                      }}
                    >
                      Delete
                    </button>
                ...
                <td> <button type="button" className="btn btn-info" onClick={() => {
                  ...
                  custDataHandler(bData.emailId);
                }}>
                  Show Details
                </button></td>

                <td> <button type="button" className="btn btn-primary" onClick={() => {
                  navigate(`/payment?eventId=${bData.eventId}`);
                }}>
                  Payment Details
                </button></td>

                <td> <button type="button" className="btn btn-light" onClick={() => {
                  navigate(`/paymentstatus?eventId=${bData.eventId}`);
                }}>
                  Change Status
                </button></td>
